...
def _upsert_user_sync(user_id: int, username: Optional[str], first_name: str, last_name: Optional[str]):
    ...
    with _get_connection() as conn:
        ...
        if exists:
            ...
            cursor.execute("""
                INSERT INTO users (user_id, username, first_name, last_name, created_at, last_active, greetings_count)
                VALUES (?, ?, ?, ?, ?, ?, 0)
            """, (user_id, username, first_name, last_name, now_str, now_str))
        conn.commit()

async def upsert_user(user_id: int, username: Optional[str], first_name: str, last_name: Optional[str]):
    """Foydalanuvchi ma'lumotlarini qo'shish yoki yangilash."""
    await asyncio.to_thread(_upsert_user_sync, user_id, username, first_name, last_name)
...
def _get_recent_users_sync(limit: int = 10) -> List[Dict[str, Any]]:
    with _get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT user_id, username, first_name, last_name, created_at, last_active, greetings_count
            FROM users
...
async def get_all_user_ids() -> List[int]:
    """Barcha foydalanuvchilar ID ro'yxati (Rassilka uchun)."""
...
def _export_users_csv_sync(filepath: str) -> str:
    with _get_connection() as conn:
        ...
        with open(filepath, mode="w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["User ID", "Username", "Ismi", "Familiyasi", "Qo'shilgan vaqti", "Oxirgi faolligi", "Tabriklar soni"])
            for r in rows:
                writer.writerow([r["user_id"], r["username"] or "", r["first_name"] or "", r["last_name"] or "", r["created_at"], r["last_active"], r["greetings_count"]])
